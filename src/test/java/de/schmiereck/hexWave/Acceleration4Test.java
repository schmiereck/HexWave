...
public class Acceleration4Test {
    ...
    public void testCalcNewVelocity() {
        ...
        for (int v = -MaxVelocity * 2; v < MaxVelocity * 2; v += 5) {
            int velocity = 10;
            int xAMove = v;
            int nv = calcNewVelocity(velocity, xAMove, MaxVelocity);
    ...
    public void testCalcNewAcceleration() {
        ...
        for (int v = -MaxVelocity * 4; v < MaxVelocity * 4; v += 5) {
            int velocity = 5;
            int acceleration = v;
            int na = calcNewAcceleration(velocity, acceleration, MaxVelocity);
            final int newVelocity = velocity + na;
            System.out.printf("newVelocity:%4d, na:%4d, velocity:%4d, acceleration:%4d\n", newVelocity, na, velocity, acceleration);
        }
    }
    ...
    private static int calcNewAcceleration(final int xVelocity, final int xAcceleration, final int MaxVelocity) {
        ...
        //int nm = Math.ceilDiv(((MaxVelocity - Math.abs(newVelocity)) * xAcceleration), MaxVelocity);
        //int nm = (int)Math.floorDiv((long)((MaxVelocity - Math.abs(newVelocity)) * xAcceleration), (long)MaxVelocity);
        ...
        if (dir > 0) {
            ...
            if (dir < 0) {
                //na = ((MaxVelocity - (xVelocity + newAcceleration)) * newAcceleration) / MaxVelocity;
                //na = ((MaxVelocity - (xVelocity)) * newAcceleration) / MaxVelocity;
                na = Math.ceilDiv(((MaxVelocity - Math.abs(xVelocity)) * newAcceleration), MaxVelocity);
            } else {
                na = 0;
            }
