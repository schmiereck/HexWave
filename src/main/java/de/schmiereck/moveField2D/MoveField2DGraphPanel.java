...
public class MoveField2DGraphPanel extends JPanel {
    private final MoveFieldArrDto fieldArrDto;
    private final int maxFieldValue;

    public MoveField2DGraphPanel(final MoveFieldArrDto fieldArrDto, final int maxFieldValue) {
    ...
    private void drawGraph(Graphics2D g2d, double xScale, double yScale, int padding, int labelPadding, int maxAmplitudeValue, int pointWidth,
                           final int pos, final MoveFieldDto fieldDto) {
        final int x = (int) (pos * xScale + padding + labelPadding);
        {
            final int y = (int) ((maxAmplitudeValue - fieldDto.moveValue) * yScale + padding);
